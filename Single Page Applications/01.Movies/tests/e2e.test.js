...
describe('E2E tests', function () {
  ...
  describe('Authentication', () => {
    it('register does not work with empty fields', async () => {
      ...
      await page.waitForSelector('.navbar-nav', { timeout: interval });

      await page.click('nav >> text=Register', { timeout: interval });
      await page.waitForSelector('#register-form', { timeout: interval });

      await page.click('#register-form >> text=Register', { timeout: interval });
    ...
    it('register makes correct API call', async () => {
      ...
      const [request] = await Promise.all([
        onRequest(),
        page.click('#register-form >> text=Register', { timeout: interval }),
      ]);
    ...
    it('login makes correct API call', async () => {
      ...
      const [request] = await Promise.all([
        onRequest(),
        page.click('#login-form >> text=Login', { timeout: interval }),
      ]);
  ...
  describe('Navigation bar', () => {
    it('guest user should see correct navigation', async () => {
      await page.goto(host);
      await page.waitForSelector('nav', { timeout: interval });
  ...
  describe('Catalog', () => {
    ...
    it('guest does NOT see edit/delete buttons', async () => {
      ...
      own(0);
      total(5);
      await page.waitForSelector('.navbar-brand', { timeout: interval });
  ...
  describe('CRUD', () => {
    ...
    it('create does NOT work with empty fields', async () => {
      ...
      await page.click('text=Add Movie', { timeout: 5000 });

      await page.waitForSelector('#add-movie-form', { timeout: interval });
      page.click('form >> text=Submit', { timeout: interval });
    ...
    it('edit should populate form with correct data', async () => {
      ...
      await page.waitForSelector('text=Edit', { timeout: interval });
      await page.click('text=Edit', { timeout: interval });

      await page.waitForSelector('#edit-movie form', { timeout: interval });

      const inputs = await page.$$eval('#edit-movie input, textarea', (elements) =>
    ...
    it('delete makes correct API call for logged in user', async () => {
      ...
      await page.waitForSelector('text="Delete"', { timeout: interval });
  ...
  describe('Like functionality', async () => {
    ...
    it('Like button should increase total likes by 1 after a click on it', async () => {
      ...
      const [request] = await Promise.all([
        onRequest(),
        page.click('.container >> text="Like"', { timeout: interval }),
      ]);
