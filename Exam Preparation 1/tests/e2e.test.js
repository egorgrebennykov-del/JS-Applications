...
describe("E2E tests", function () {
    ...
    this.timeout(DEBUG ? 120000 : timeout);
        ...
        if (fs.existsSync(resultsDir)) {
            ...
            console.log('🧹 Cleaned up previous test-results folder');
        }

        // Запускаем браузер (параметры можно менять)
    ...
    // ---------- beforeEach – создаём контекст, страницу, подключаем лог консоли ----------
    beforeEach(async function () {
        this.timeout(10000);

        // Создаём новый контекст (без видео, если не нужно)
        context = await browser.newContext();
        // Если в проекте есть функция моков – вызываем её
        if (typeof setupContext === 'function') setupContext(context);
        ...
        global.consoleLogs = [];
        page.on('console', msg => {
            global.consoleLogs.push(`[${msg.type()}] ${msg.text()}`);
        });
    });

    // ---------- afterEach – сохраняем артефакты при падении ----------
    afterEach(async function () {
        const testFailed = this.currentTest && this.currentTest.state === 'failed';
        ...
        if (testFailed && page) {
            ...
            try {
                ...
                fs.writeFileSync(path.join(testDir, 'screenshot.png'), screenshot);
                console.log(`✅ Screenshot saved: ${path.join(testDir, 'screenshot.png')}`);

                // 2. HTML страницы
                const html = await page.content();
                fs.writeFileSync(path.join(testDir, 'page.html'), html);
                console.log(`✅ HTML saved: ${path.join(testDir, 'page.html')}`);

                // 3. Логи консоли
                if (global.consoleLogs && global.consoleLogs.length) {
                    fs.writeFileSync(path.join(testDir, 'console.log'), global.consoleLogs.join('\n'));
                    console.log(`✅ Console logs saved: ${path.join(testDir, 'console.log')}`);
                }
            } catch (err) {
                console.error('❌ Failed to capture artifacts:', err.message);
            }
        }
    ...
    describe("Navigation bar [ 10 Points ]", () => {
        ...
        it("Guest user navigation should work [ 2.5 Points ]", async function () {
            ...
            let motorcyclesBtn = await page.waitForSelector('nav >> text=Motorcycles', { timeout: interval });
            await motorcyclesBtn.click();
    ...
    describe("Home Page [ 5 Points ]", function () {
        it("Show Home page text [ 2.5 Points ]", async function () {
            await page.goto(host);
            await page.waitForSelector('text=Unleash the spirit of speed with Samurider - where samurai soul meets Japanese engineering.', { timeout: interval });
            expect(await page.isVisible("text=Unleash the spirit of speed with Samurider - where samurai soul meets Japanese engineering.")).to.be.true;
        });
    ...
    describe("Dashboard Page [ 15 Points ]", function () {
        it("Show Motorcycles page - welcome message [ 2.5 Points ]", async function () {
            ...
            await page.waitForSelector('main >> text=Available Motorcycles', { timeout: interval });
            expect(await page.isVisible("main >> text=Available Motorcycles")).to.be.true;
        });
        ...
        it("Check Motorcycles page with 0 Motorcycles [ 2.5 Points ]", async function () {
            ...
            await page.waitForSelector('text=No avaliable motorcycles yet.', { timeout: interval });
                // Вот так правильно получить значение из браузера:
                const allMoto = await page.evaluate(() => window.allMoto);
                 console.log('allMoto from browser:', allMoto);
                 //
    ...
    describe("CRUD [ 50 Points ]", () => {
        ...
        describe('Edit [ 17.5 Points ]', function () {
            ...
            it("Edit redirects to Details on success [ 2.5 Points ]", async function () {
                ...
                let motorcyclesBtn = await page.waitForSelector('nav >> text=Motorcycle', { timeout: interval });
                await motorcyclesBtn.click();
